...
namespace kiwi::parse {
    ...
    auto load_config(const std::FilePath& config_folder) -> Config {
        ...
        serde::deserialize(serde::Json::load_from(config_folder / "config.json"), config_paths);
    ...
    try {
        ...
        if (extension == ".json") {
            ...
            for (const auto& row : workbook.sheet_by_index(0).rows()) {
                ...
                iter->second.emplace_back(std::move(input), std::move(output));
            }
        } else {
        ...
        debug::exception_in("Load connections config", err.what());
    }
